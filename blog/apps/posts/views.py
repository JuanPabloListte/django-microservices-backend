...
class ListPostView(StandardAPIView):
    def get(self, request, *args, **kwargs):
        cache_key = 'published_posts'
        posts_shown = cache.get(cache_key)
        
        if not posts_shown: 
            posts_shown = Post.postobjects
            cache.set(cache_key, posts_shown, 900) #Cache for 15 min
...
class DetailPostView(StandardAPIView):
    def get(self, request, id, *args, **kwargs):
        ...
        if not post:
            ...
            cache.set(cache_key, post, 900)  # Cache for 15 min
        
        return self.send_response(PostSerializer(post))
...
class CreatePostView(StandardAPIView):
    def post(self, request, format=None):
        ...
        if data.get('user').get('role') != 'editor':
            raise PermissionError("You don't have permissions to create a post.")        
